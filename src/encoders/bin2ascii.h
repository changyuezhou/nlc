...
inline std::string hex2bin(const std::string &s) {
  if (s.size() % 2) throw std::runtime_error("Odd hex data size");
...
inline std::string b64_encode(const std::string &s) {
  ...
  for (size_t i = 0; i < (3 - s.size() % 3) % 3; ++i) r.push_back('=');
  return r;
}
...
inline std::string b64_decode(const std::string &s) {
  ...
  std::string r;
  if (!s.size()) return r;
  if (s.size() % 4) throw std::runtime_error("Invalid base64 data size");
  size_t pad = 0;
  if (s[s.size() - 1] == '=') pad++;
  if (s[s.size() - 2] == '=') pad++;
