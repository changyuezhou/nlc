...
void MD5::update (uint1 const*input, uint4 input_length) {
...
void MD5::update(std::istream & stream){
  ...
  while (stream.good()){
    stream.read((char*)(&buf[0]), 1024); // note that return value of read is unusable.
    len=stream.gcount();
    update(buf, len);
  }
...
MD5::MD5(const string & str) {
  init();
  unsigned char const * tmp = (unsigned char const *)str.c_str();
  update(tmp,str.length());
  finalize();
}

MD5::MD5(unsigned char const*str)
{
    init();
...
std::ostream& operator<<(std::ostream &stream, MD5 const& context){
...
// a multiple of 4.
void MD5::encode (uint1 *output, uint4 const*input, uint4 len) {
...
// a multiple of 4.
void MD5::decode (uint4 *output, uint1 const*input, uint4 len){
...
void MD5::memcpy (uint1 *output, uint1 const*input, uint4 len){
